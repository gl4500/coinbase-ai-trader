...
- **2026-05-24: PnL-anchored trail with model-driven exits (`agents/exit_thresholds.py`)** — three-trigger exit design. **TRAIL_STOP (gain-proportional):** exit at `peak_pnl_pct - giveback`, where giveback = `max(peak_pnl_pct * GIVEBACK_FRAC, 2 * FEE_RATE)`. **MODEL_DOWN (NEW):** exit when v4.5 `p_down > 0.55` (cached on `pos[p_down]` by generate_signal); uses model's strongest signal. **STOP_LOSS:** unchanged -8% backstop. **Layer 2 capital-cap:** large positions (>max($200, 5% of capital)) get tighter giveback. Operator framing: "if I'm up $5 I want to lose $0.06, not 6% of price". Constants: `FEE_RATE=0.006 GIVEBACK_FRAC=0.10 LARGE_POSITION_FRAC=0.05 LARGE_POSITION_FLOOR=200 MAX_DOLLAR_GIVEBACK_FRAC=0.02 MAX_LOSS_FRAC_OF_CAPITAL=0.005 _P_DOWN_EXIT_THRESHOLD=0.55`. State additions: `peak_pnl_pct position_dollars p_down`; `_CNNBook._migrate_position_state` handles legacy. 56 new/updated tests (18 unit + 3 migration + 5 risk-exits + 11 WS + worked examples). 24h backtest: +$11.27 realized vs $0 with old peak-price trail. Spec: `docs/superpowers/specs/2026-05-23-pnl-anchored-trail-design.md`. Plan: `docs/superpowers/plans/2026-05-23-pnl-anchored-trail.md`.
