...
- **2026-05-24: Dashboard follow-up (#52 polish + Chart tab + 24h enrichment)** — three fixes building on `feat/dashboard-shadow-week-monitoring`:
  - **`/api/compare` now uses HTTP fallback to `/api/agents/status`** instead of reading the DB. Fixes the bug where ComparisonHeader showed `$0` for 8002: the dev DB has no `agent_state` row until first trade closes, but the running backend has a live `$1000` balance in memory. New helper `_fetch_live_agent_state(port)`.
  - **Retired the top-level "Performance" tab + `FiringCounter` block** from `App.tsx`. PerformanceDashboard now lives only under `Agents → Performance` sub-tab (where AgentsDashboard already embeds it). Cleaner top-level nav. EquityCurve chart still reachable via Agents → Performance.
  - **NEW "Chart" tab + `<PriceChart>` component** using TradingView's free widget embed. Selectable by Coinbase product_id (BTC-USD, ETH-USD, etc.), with 15m/1h/4h/1D timeframe buttons. Uses TradingView's Coinbase feed; iframe-based, zero deps. `COINBASE:{BASE}{QUOTE}` symbol mapping.
  - **24h high/low/pct-change enrichment** in `/api/products`. Coinbase Advanced Trade's products endpoint doesn't supply `high_24h`/`low_24h` (the DB columns stayed at default `0`). New `_enrich_products_with_24h` helper computes them from the last 24 hourly candles per pid and overrides the dict before return. Fixes MarketBrowser's 24h-high/low columns showing all zeros.
  - 3 new tests in `test_shadow_monitoring.py::TestFetchLiveAgentState` covering unreachable port, OK response extraction, and non-200 handling.

- **2026-05-24: Shadow-week monitoring (#52 + #54)** — new dashboard UX comparing the production v3 driver (8001/coinbase.db) vs the v4.5 shadow driver (8002/coinbase_dev.db) head-to-head. **`ComparisonHeader.tsx`** is a sticky strip rendered below the main app header showing both backends side-by-side: balance, realized PnL, open positions, unrealized PnL estimate (from cached `position_dollars`), and the delta (v4.5 − v3). Polls `/api/compare` every 30s. **`EquityCurve.tsx`** renders an inline SVG line chart of cumulative realized PnL over the last N days (default 7), two overlaid series (cyan v3, amber v4.5). Renders on the Performance Dashboard tab. Two new backend endpoints in `main.py`: `GET /api/compare` (reads `agent_state` from both DBs) and `GET /api/equity_curve?days=N` (reads `trades` from both DBs, computes cumulative). Both endpoints fail gracefully when a DB is missing (returns null / empty array). 11 new unit tests in `tests/test_shadow_monitoring.py` covering both endpoints. Solves the "watch v3 and v4.5 daily" workflow without requiring SQL queries — operator glances at the sticky strip + equity chart for the entire shadow-week comparison.
- **2026-05-23: WS-driven exit checker (`agents/exit_watcher.py`)** — new module registers an async price-tick handler on the existing Coinbase WS ticker channel. Fires `WS_TRAIL_STOP` / `WS_STOP_LOSS` exits on held positions in sub-second latency instead of waiting for the 60s scan cycle. Max-hold (7-day) exit stays on the scan loop where 60s cadence is appropriate.
  ...
  - `main.py` lifespan — calls `attach_exit_watcher(app_state.ws_subscriber, app_state.cnn_agent.book)` after `ws_subscriber.start()`. Logs `WS exit watcher attached`.
  - New invariant **#18** in `CLAUDE.md` — WS exit-handler isolation: `on_price_tick` must catch every exception in its body and log at ERROR; unretrieved-Task warnings hide errors otherwise.
  - **Tests:** `tests/test_exit_watcher.py` NEW (6 unit + 1 integration). `tests/test_cnn_agent.py`: +1 race test (`TestCNNBookSellLock::test_sell_lock_serializes_concurrent_callers`). `tests/test_cnn_risk_exits.py`: +1 cache-write test (`test_check_risk_exits_writes_trail_pct_to_position`). Total +9 tests.
  - **Verify:** `cd backend && python -m pytest tests/test_exit_watcher.py tests/test_cnn_agent.py::TestCNNBookSellLock tests/test_cnn_risk_exits.py::test_check_risk_exits_writes_trail_pct_to_position -v` — 9/9 PASS expected.
  - **Telemetry:** trigger strings `WS_TRAIL_STOP` / `WS_STOP_LOSS` distinguish WS-driven from scan-driven `TRAIL_STOP` / `STOP_LOSS`. After 24h, query `trades` table by `trigger_close` to measure latency-reduction effect.
  - **Deployment:** Operator restarts 8001 to pick up the lifespan change. Verify `WS exit watcher attached` log line on startup.
  - **Spec:** `docs/superpowers/specs/2026-05-23-ws-exit-checker-design.md`. **Plan:** `docs/superpowers/plans/2026-05-23-ws-exit-checker.md`.
...
- **2026-05-23: Strategy-discovery rebuild Phase 1 Task 6** — bulk marketcap backfill orchestrator (`tools/strategy_discovery/build_universe_marketcap.py`). Reads Task 5's universe JSON, identifies pids needing backfill (< min-days coverage), dispatches `fetch_marketcap_history` for each, persists via `build_marketcap_parquet._save_marketcap_history`. Skips already-covered pids (data-first directive). Phase 1 implementation complete — operator runs integration sequence next.
- **2026-05-23: Strategy-discovery rebuild Phase 1 Task 5** — universe curation script (`tools/strategy_discovery/curate_universe.py`). Pure ranking helpers (`rank_by_market_cap`, `rank_by_fdv_mc_ratio`, `rank_by_turnover`, `curate_universe`) + CLI orchestrator that consumes Task 1's inventory + Task 3's supply parquet + existing marketcap parquets, applies Q5 criteria (15 large / 15 mid / 10 high FDV-MC / 10 low turnover, ≥6 months history), writes `docs/superpowers/specs/2026-05-23-universe-50.{md,json}`. The CLI is operator-driven and runs after supply + marketcap parquets are populated for the candidate pool (deferred to Phase 1 integration step).
- **2026-05-23: Strategy-discovery rebuild Phase 1 Task 4** — extended `services/coinpaprika_marketcap._PRODUCT_TO_CP_ID` from 28 to 189 entries. Sourced from Task 1's inventory (longest-history pids first), looked up via CoinPaprika's public `/v1/coins` endpoint (one call, no key). Skipped stables/wrapped tokens (USDC, WBTC, STETH, etc.) and gold-pegged (PAXG). 161 net new entries added; 3 stables skipped; 4 pids unmatched (CORECHAIN, FUN1, JUPITER, LIGHTER). Feeds the candidate pool that Task 5 curates to 50 pids. New test: `test_product_to_cp_id_has_at_least_100_entries`.
