...
- **2026-05-23: GPU-batched XGB v4.5 feature kernel (`tools/xgb_v4_5_features_batch.py`)** — new module vectorizes feature extraction across all sample bars per PID via PyTorch `tensor.unfold` strided windows; runs on CPU or CUDA via `device` arg.
  ...
  - **Numerical contract:** max abs diff < 1e-4 vs per-bar numpy reference on the (N, 210) feature matrix (CPU-batched); < 1e-3 cross-device (GPU-batched vs CPU per-bar) — drift comes from floating-point reduction order, not algorithmic difference. Labels + timestamps must match exactly. Bollinger bands computed local-per-slice (matching `extract_v4_5` semantics), not globally, to preserve inference/train parity since `agents/xgb_signal.py` continues using the per-bar numpy path.
  - **Tests:** `tests/test_xgb_v4_5_features_batch.py` NEW — 14 tests: per-stat parity (`last`, `mean`+`std`, `slope`, `min`+`max`, `pct_rank`, `dlt5/10/30`), local-BB parity, triple-barrier vectorization parity, `_build_pid_ohlcv` round-trip, full-pipeline CPU parity, full-pipeline CUDA parity (skipif no CUDA), empty/too-few-candle guards.
  - **Verify:** `cd backend && python -m pytest tests/test_xgb_v4_5_features_batch.py -v` — 13/13 PASS on non-CUDA boxes (CUDA test skipped); 14/14 PASS on RTX 2060 box.
  - **Out of scope (deferred):** GPU port of `agents/xgb_signal.py` inference path (per-tick single-bar; GPU launch overhead > numpy). Multi-PID GPU batching (~30 min target acceptable). GPU port of `train_xgb_v4.py` (v4 2-class trainer; v4.5 is the only active horizon-sweep workflow).
  - **Deployment:** Operator pauses 8001 → runs full pytest → atomic commit + push → runs sweep with `--device cuda`. Per the spec's risk register, first GPU-trained horizon should be compared against the prior CPU-trained baseline in `v4_5_horizon_compare` HTML before promoting.
  - **Spec:** `docs/superpowers/specs/2026-05-23-gpu-batched-feature-kernel-design.md`. **Plan:** `docs/superpowers/plans/2026-05-23-gpu-batched-feature-kernel.md`.
