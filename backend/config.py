...
def _validate_backend(value: str) -> str:
    ...
    if value not in _VALID_BACKENDS:
        raise ValueError(f"MODEL_BACKEND={value!r} invalid. Valid: {sorted(_VALID_BACKENDS)}")
    return value
...
class Config:
    ...
    coinbase_api_secret: str = field(
        default_factory=lambda: os.getenv("COINBASE_API_PRIVATE_KEY", "")
    )
    ...
    max_tracked_products: int = field(
        default_factory=lambda: int(os.getenv("MAX_TRACKED_PRODUCTS", "100"))
    )
    ...
    llm_skip_threshold: float = field(
        default_factory=lambda: float(os.getenv("LLM_SKIP_THRESHOLD", "0.75"))
    )
    # Seconds to sleep between products during a scan (reduce to speed up, raise to avoid rate limits)
    scan_sleep_secs: float = field(
        default_factory=lambda: float(os.getenv("SCAN_SLEEP_SECS", "0.1"))
    )
    ...
    max_position_usd: float = field(
        default_factory=lambda: float(os.getenv("MAX_POSITION_USD", "500"))
    )
    max_total_exposure: float = field(
        default_factory=lambda: float(os.getenv("MAX_TOTAL_EXPOSURE", "5000"))
    )
    min_volume_24h: float = field(
        default_factory=lambda: float(os.getenv("MIN_VOLUME_24H", "1000000"))
    )
    # ATR-based position sizing
    atr_risk_pct: float = field(
        default_factory=lambda: float(os.getenv("ATR_RISK_PCT", "0.01"))
    )  # 1% account per trade
    atr_multiplier: float = field(
        default_factory=lambda: float(os.getenv("ATR_MULTIPLIER", "2.0"))
    )  # stop = 2× ATR

    # ── Drawdown circuit breaker ───────────────────────────────────────────────
    daily_drawdown_limit: float = field(
        default_factory=lambda: float(os.getenv("DAILY_DRAWDOWN_LIMIT", "0.05"))
    )  # 5%
    weekly_drawdown_limit: float = field(
        default_factory=lambda: float(os.getenv("WEEKLY_DRAWDOWN_LIMIT", "0.10"))
    )  # 10%
    ...
    min_signal_strength: float = field(
        default_factory=lambda: float(os.getenv("MIN_SIGNAL_STRENGTH", "0.20"))
    )
    ...
    cnn_buy_threshold: float = field(
        default_factory=lambda: float(os.getenv("CNN_BUY_THRESHOLD", "0.60"))
    )
    cnn_sell_threshold: float = field(
        default_factory=lambda: float(os.getenv("CNN_SELL_THRESHOLD", "0.40"))
    )
    ...
    cnn_train_every_n_scans: int = field(
        default_factory=lambda: int(os.getenv("CNN_TRAIN_EVERY_N_SCANS", "8"))
    )
    ...
    scan_interval_secs: int = field(
        default_factory=lambda: int(os.getenv("SCAN_INTERVAL_SECS", "900"))
    )
    ...
    model_backend: str = field(
        default_factory=lambda: _validate_backend(os.getenv("MODEL_BACKEND", "xgb").lower())
    )
    ...
    xgb_v45_thresh_up: float = field(
        default_factory=lambda: float(os.getenv("XGB_V45_THRESH_UP", "0.50"))
    )
    xgb_v45_thresh_down: float = field(
    ...
    backfill_interval_hours: int = field(
        default_factory=lambda: int(os.getenv("BACKFILL_INTERVAL_HOURS", "24"))
    )
    # How many days of history to fetch for a brand-new product
    backfill_new_product_days: int = field(
        default_factory=lambda: int(os.getenv("BACKFILL_NEW_PRODUCT_DAYS", "365"))
    )
