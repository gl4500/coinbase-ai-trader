...
def render_scorecard(per_cap: List[CapScorecard]) -> str:
    ...
    lines.append("")
    lines.append(
        "| cap | deflated profit | raw profit | inflation | max DD | sortino | trades | pct full | mean conc | pass? |"
    )
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|:---:|")
    ...
    for c in sorted(per_cap, key=lambda x: x.cap):
        ...
        lines.append("")
        lines.append(
            f"- Deflated cumulative profit: **{c.metrics.cumulative_profit_deflated:+.4f}**  (raw {c.metrics.cumulative_profit_raw:+.4f}, inflation {c.inflation:.4f})"
        )
        ...
        lines.append(
            f"- Slot utilization: {c.metrics.pct_slots_full:.2%} full, mean concurrent {c.metrics.mean_concurrent:.2f}"
        )
        lines.append(f"- K subsets evaluated: {c.k_evaluated}")
        lines.append(
            "- Gates: " + ", ".join(f"{k}={'✅' if v else '❌'}" for k, v in c.gates.items())
        )
