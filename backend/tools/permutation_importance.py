...
def _load_model():
    ...
    from agents.cnn_agent import (
        _ARCH_REGISTRY, _active_arch, _model_path_for, N_CHANNELS, _DEVICE,
    )
    arch = _active_arch()
...
CHANNEL_NAMES: Tuple[str, ...] = (
    ...
    "ivrv_60",      "vol_sentiment",
)


@torch.no_grad()
def _val_bce(model, X: torch.Tensor, y: torch.Tensor, w: torch.Tensor,
             batch: int = 256) -> float:
...
def main(k_seeds: int = 5, out_dir: str | None = None) -> str:
    ...
    for rk, r in enumerate(sample_rows, 1):
        ...
        flag = (
            "  HEAVY"    if r["delta"] >  0.005 else
            "  used"     if r["delta"] >  0.001 else
            "  marginal" if r["delta"] >  0.0   else
            "  NOISE"
        )
        print(
