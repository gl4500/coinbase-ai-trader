...
def test_dollar_cap_floor_skips_small_positions():
    assert _dollar_cap_floor(peak_pnl_pct=0.05, position_dollars=50.0, total_capital=1000.0) is None
    # Exactly at threshold — not strictly > → skip
    assert (
        _dollar_cap_floor(peak_pnl_pct=0.05, position_dollars=200.0, total_capital=1000.0) is None
    )
...
def test_dollar_cap_floor_engages_for_large_positions():
    ...
    assert _dollar_cap_floor(
        peak_pnl_pct=0.05, position_dollars=250.0, total_capital=1000.0
    ) == pytest.approx(0.03)
...
def test_dollar_cap_floor_capital_loss_cap_tighter_at_concentrated_position():
    ...
    assert _dollar_cap_floor(
        peak_pnl_pct=0.05, position_dollars=500.0, total_capital=1000.0
    ) == pytest.approx(0.04)
...
def test_dollar_cap_floor_threshold_scales_with_capital():
    ...
    assert (
        _dollar_cap_floor(peak_pnl_pct=0.05, position_dollars=500.0, total_capital=1000.0)
        is not None
    )
    # capital $10k:  threshold=$500, position EQ threshold → skips
    assert (
        _dollar_cap_floor(peak_pnl_pct=0.05, position_dollars=500.0, total_capital=10_000.0) is None
    )
    # capital $100k: threshold=$5k, position $500 << threshold → skips
    assert (
        _dollar_cap_floor(peak_pnl_pct=0.05, position_dollars=500.0, total_capital=100_000.0)
        is None
    )
...
def test_compute_exit_threshold_layer2_tightens_over_layer1():
    ...
    result = _compute_exit_threshold(
        peak_pnl_pct=0.05, atr_pct=0.06, position_dollars=500.0, total_capital=1000.0
    )
...
def test_compute_exit_threshold_layer2_skipped_when_capital_omitted():
    ...
    result = _compute_exit_threshold(peak_pnl_pct=0.05, atr_pct=0.06, position_dollars=500.0)
    assert result == pytest.approx(0.038)
...
def test_compute_exit_threshold_worked_example_dash():
    ...
    # exit at +6.6% PnL
    result = _compute_exit_threshold(
        peak_pnl_pct=0.078, atr_pct=0.06, position_dollars=58.0, total_capital=1000.0
    )
...
def test_compute_exit_threshold_worked_example_paxg_never_green():
    ...
    result = _compute_exit_threshold(
        peak_pnl_pct=0.0, atr_pct=0.04, position_dollars=452.0, total_capital=1000.0
    )
    assert result == float("-inf")
...
def test_compute_exit_threshold_eth_locks_meaningful_gain():
    ...
    result = _compute_exit_threshold(
        peak_pnl_pct=0.043, atr_pct=0.06, position_dollars=103.0, total_capital=1000.0
    )
