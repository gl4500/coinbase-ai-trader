...
def test_relabel_at_horizon_matches_triple_barrier_oracle():
    ...
    sample_ts = np.array([candles[i]["start"] for i in sample_indices], dtype=np.int64)

    expected = [
        _label_triple_barrier(candles, i, forward_hours, up_mult, dn_mult, label_thresh)
    ...
    for j, exp in enumerate(expected):
        if exp is None:
            assert not mask[j], f"sample {j} (idx={sample_indices[j]}) expected None → mask=False"
        else:
            assert mask[j], f"sample {j} (idx={sample_indices[j]}) expected {exp} → mask=True"
            assert y[j] == pytest.approx(exp), f"sample {j}: got y={y[j]}, expected {exp}"
...
def test_relabel_at_horizon_marks_unmappable_ts_as_invalid():
    ...
    assert bool(mask[0]) in (True, False)  # depends on labeler; both ok
    assert not mask[1], "unmappable ts must yield mask=False"
