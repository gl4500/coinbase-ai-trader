...
def test_missing_volume_drops_candidate_row():
    ...
    out = stamp_tokenomic(
        df_hourly, df_daily_with_gap, _trivial_supply(), drop_on_missing_volume=True
    )
...
def test_fdv_derived_from_price_and_total_supply():
    ...
    np.testing.assert_allclose(out["fdv"].to_numpy(), np.array([2.0, 4.0, 8.0]) * 25_000.0)
    np.testing.assert_allclose(
        out["fdv_over_mc"].to_numpy(), (np.array([2.0, 4.0, 8.0]) * 25_000.0) / 50_000.0
    )
    # circ_over_total is a constant row-derived value: 10_000 / 25_000 = 0.4
    np.testing.assert_allclose(out["circ_over_total"].to_numpy(), np.full(3, 0.4))
    np.testing.assert_allclose(out["vol_over_mc"].to_numpy(), np.full(3, 1_000.0 / 50_000.0))
