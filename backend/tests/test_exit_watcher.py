...
def _make_pos(
    *, avg=100.0, peak=100.0, trail=0.06, peak_pnl_pct=None, position_dollars=None, size=1.0
):
...
class TestOnPriceTickPnLAnchored:
    ...
    async def test_on_price_tick_does_not_fire_ws_model_down_when_p_down_stale(self):
        ...
        book.positions["ABC-USD"]["p_down_ts_ms"] = (
            int(time.time() * 1000) - _P_DOWN_STALE_MS - 1000
        )
