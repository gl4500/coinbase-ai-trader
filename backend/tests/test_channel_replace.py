...
class TestRunReplace:
    ...
    def test_returns_mean_auc_and_delta(self):
        ...
        result = run_replace(X, y, ts, channel_idx=15, replacement=sig,
                             n_folds=2, embargo_hours=0, n_estimators=10)
        assert set(result.keys()) >= {"channel", "baseline_auc",
                                       "replaced_auc", "delta"}
    ...
    def test_replacing_with_same_data_yields_zero_delta(self):
        ...
        ts = (np.arange(n) * 3600).astype(np.int64)
        sig = X[:, 5, :].copy()
        result = run_replace(X, y, ts, channel_idx=5, replacement=sig,
                             n_folds=2, embargo_hours=0, n_estimators=10)
