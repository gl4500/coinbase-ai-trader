...
# TestInferenceRegimeGate, TestSuppressionsGatedByBackend, TestLLMSkippedUnderXgb
# all DELETED #311-refactor-f — the gates/branches they exercised (HMM regime
# block, Hurst random-walk, LightGBM filter, Ollama LLM blend) were the dead
# CNN-backend-only branches removed in this module. Policy lock for the
# deletions lives in backend/tests/test_config.py::TestNoCnnBackendOnlyBranches.
...
class TestMakerExecutionRouting:
    ...
    async def test_taker_path_when_flag_off(self, agent):
        ...
        with (
            patch.object(_cnn_mod.config, "use_maker_execution", False, create=True),
            patch.object(_cnn_mod.coinbase_client, "get_best_bid_ask", new=AsyncMock()) as mock_bba,
        ):
    ...
    async def test_maker_path_populates_quotes_and_routes(self, agent):
        ...
        with (
            patch.object(_cnn_mod.config, "use_maker_execution", True, create=True),
            patch.object(
                _cnn_mod.coinbase_client, "get_best_bid_ask", new=AsyncMock(return_value=quotes)
            ) as mock_bba,
        ):
            result = await agent._execute_live_order(executor, signal)
