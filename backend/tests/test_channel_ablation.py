...
much does mean_auc move vs the full 28-channel baseline?" Useful to
...
class TestZeroChannel:
    def test_zeros_target_channel_only(self):
        ...
        X = np.ones((4, 28, 60), dtype=np.float32)
    ...
    def test_does_not_mutate_input(self):
        ...
        X = np.full((2, 28, 60), 7.0, dtype=np.float32)
        original = X.copy()
        _ = _zero_channel(X, 0)
...
class TestRunAblation:
    def test_returns_one_row_per_channel(self):
        """Smoke test on tiny synthetic input — 28 channels in, 28 rows out."""
        ...
        results = run_ablation(X, y, ts, n_folds=2, embargo_hours=0, n_estimators=10)
        assert "baseline_auc" in results
        assert "rows" in results
        assert len(results["rows"]) == 28
        for row in results["rows"]:
            assert set(row.keys()) >= {"channel", "mean_auc", "delta"}
            assert 0 <= row["channel"] < 28
