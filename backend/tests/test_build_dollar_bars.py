...
from tools.build_dollar_bars import calibrate_threshold, candle_dollar_value


def test_candle_dollar_value_uses_typical_price():
    # typical price = (high + low + close) / 3 = (110 + 90 + 100) / 3 = 100
    c = {"start": 0, "open": 95.0, "high": 110.0, "low": 90.0, "close": 100.0, "volume": 4.0}
...
def test_calibrate_threshold_is_total_over_1h_count():
    ...
    candles = [
        {"start": i, "open": 100.0, "high": 100.0, "low": 100.0, "close": 100.0, "volume": 3.0}
        for i in range(3)
    ]
...
def test_dollar_bars_ohlc_aggregation():
    candles = [
        {"start": 0, "open": 100.0, "high": 110.0, "low": 95.0, "close": 105.0, "volume": 10.0},
        {"start": 60, "open": 105.0, "high": 120.0, "low": 100.0, "close": 115.0, "volume": 10.0},
        {"start": 120, "open": 115.0, "high": 118.0, "low": 90.0, "close": 92.0, "volume": 10.0},
    ]
...
def test_build_for_pid_writes_parquet(tmp_path, monkeypatch):
    ...
    one_min = [
        {"start": s, "open": 100.0, "high": 100.0, "low": 100.0, "close": 100.0, "volume": 3.0}
    ...
    monkeypatch.setattr(bdb, "_dollar_parquet_path", lambda pid: str(tmp_path / f"{pid}.parquet"))
