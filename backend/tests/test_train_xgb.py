...
def synthetic_dataset():
    ...
    timestamps = np.arange(
        1_700_000_000, 1_700_000_000 + N_SAMPLES * 3600, 3600, dtype=np.int64
    )
...
class TestOutput:
    ...
    def test_returns_dict_with_expected_keys(self, synthetic_dataset, tmp_path):
        ...
        result = train_xgb(
            samples, labels, ts,
            n_folds=3, embargo_hours=4,
            grid=[{"max_depth": 3, "min_child_weight": 1, "subsample": 1.0}],
            out_dir=tmp_path, n_estimators=20, seed=0,
        )
        for k in ("best_params", "fold_aucs", "mean_auc", "feature_names",
...
class TestGridSearch:
    ...
    def test_best_params_come_from_grid(self, synthetic_dataset, tmp_path):
        ...
        result = train_xgb(
            samples, labels, ts, n_folds=3,
            grid=grid, out_dir=tmp_path, n_estimators=20, seed=0,
        )
        assert result["best_params"] in grid

    def test_signal_is_learnable_mean_auc_above_random(
        ...
        result = train_xgb(
            ...
            out_dir=tmp_path, n_estimators=50, seed=0,
        )
        assert result["mean_auc"] > 0.6, (
            f"Signal not learned: mean_auc={result['mean_auc']:.3f}"
        )
