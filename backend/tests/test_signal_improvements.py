...
def _sine_prices(n: int = 200, period: float = 40.0, base: float = 100.0, amp: float = 5.0):
...
class TestMACDFastParams:
    ...
    def test_feature_builder_ch5_uses_fast_macd(self):
        ...
        """
        from agents.cnn_agent import FeatureBuilder
        ...
        assert any(v != 0.0 for v in last_vals), (
            "Ch 5 all zeros with 25 candles — old (12,26,9) params still in use"
        )
...
class TestNewChannels:
    ...
    def test_channel_21_none_btc_gives_zero(self):
        ...
        assert all(v == 0.0 for v in channels[21]), "No BTC data → Ch 21 should be all 0.0"
