...
def _macd(
    closes: List[float], fast: int = 5, slow: int = 13, signal: int = 3
    ...
    macd_line = [f - s for f, s in zip(ema_fast[diff:], ema_slow, strict=False)]
...
def _adx(
    ...
    di_plus = [100 * p / a if a > 0 else 0 for p, a in zip(dmp_s, atr_s, strict=False)]
    di_minus = [100 * m / a if a > 0 else 0 for m, a in zip(dmm_s, atr_s, strict=False)]
    dx = [
        abs(p - m) / (p + m) * 100 if (p + m) > 0 else 0
        for p, m in zip(di_plus, di_minus, strict=False)
    ]
...
def _mfi(
    ...
    if len(closes) < period + 1:
        return 50.0
    tp = [(h + lo + c) / 3 for h, lo, c in zip(highs, lows, closes, strict=False)]
    rmf = [t * v for t, v in zip(tp, volumes, strict=False)]
    pos = sum(rmf[i] for i in range(len(tp) - period, len(tp)) if i > 0 and tp[i] > tp[i - 1])
    neg = sum(rmf[i] for i in range(len(tp) - period, len(tp)) if i > 0 and tp[i] <= tp[i - 1])
    return 100.0 if neg == 0 else 100 - (100 / (1 + pos / neg))


def _obv_slope(closes: List[float], volumes: List[float], period: int = 10) -> float:
    """
...
def _vwap(
    ...
    c = closes[-period:]
    v = volumes[-period:]
    tp = [(hi + lo + cl) / 3 for hi, lo, cl in zip(h, low, c, strict=False)]
    vol_sum = sum(v)
    if vol_sum == 0:
        return c[-1], 0.0
    vwap = sum(t * vi for t, vi in zip(tp, v, strict=False)) / vol_sum
...
def _multi_rsi(
    ...
    if len(closes) < min_needed:
        return {"rsi6": 50.0, "rsi12": 50.0, "rsi24": 50.0, "buy_votes": 0, "sell_votes": 0}
...
def _realized_vol(closes: List[float], window: int = 20, annualize_days: int = 365) -> float:
    """
...
def _stoch_rsi(
    closes: List[float], period: int = 14, k_smooth: int = 3, d_smooth: int = 3
) -> Tuple[float, float]:
    """
...
class SignalGenerator:
    ...
    async def generate_signal(self, product: Dict) -> Optional[Dict]:
        ...
        if rsi_val < config.rsi_oversold:
            ...
            sell_score += (
                rsi_weight * (rsi_val - config.rsi_overbought) / (100 - config.rsi_overbought)
            )
