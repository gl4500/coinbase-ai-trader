...
LGBM_GATE_THRESHOLD = 0.52  # Predicted win prob must exceed this to allow BUY
...
class LGBMFilter:
    ...
    def is_ready(self) -> bool:
        """True once trained on at least MIN_SAMPLES rows."""
        return self._model is not None and self._n_samples >= MIN_SAMPLES
    ...
    def train(self, rows: List[Dict]) -> Optional[Dict]:
        ...
        if len(set(y_tr.tolist())) < 2 or not set(y_val.tolist()).issubset(set(y_tr.tolist())):
            X_val, y_val = X_tr[:0], y_tr[:0]  # empty — disables eval_set
